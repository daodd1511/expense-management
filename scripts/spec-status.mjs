...
const CURRENT_RE =
  /^- Current phase: (All phases complete|\d+ — (?:pending|in-progress|done|done-with-debt))$/;
...
export function parseStatus(file, text) {
  ...
  if (items.length < 3)
    fail(file, "STATUS block needs Current phase, at least one Phase, and Verification debt lines");
  ...
  if (!debtMatch)
    fail(
      file,
      `last STATUS item must be a Verification debt line, got: "${items[items.length - 1]}"`,
    );
  ...
  for (const item of items.slice(1, -1)) {
    ...
    if (!m)
      fail(
        file,
        `bad Phase line (state must be one of ${STATES.join("|")}, no backticks): "${item}"`,
      );
...
export function planMarker(planText) {
  const m = planText
    .split("\n")
    .slice(0, 10)
    .find((l) => /^status: (reference|done)$/.test(l.trim()));
...
export function collectSpecRecords(specsDir) {
  ...
  for (const slug of readdirSync(specsDir).sort()) {
    ...
    const { phases, debt } = parseStatus(
      `docs/specs/${slug}/EXECUTION.md`,
      readFileSync(execPath, "utf8"),
    );
    const doneCount = phases.filter(
      (p) => p.state === "done" || p.state === "done-with-debt",
    ).length;
