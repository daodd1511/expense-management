...
export function TransactionOverlaySheet({ variant }: { variant: 'mobile' | 'desktop' }) {
  ...
  const editing =
    state?.mode === 'edit' ? transactions.find((transaction) => transaction.id === state.transactionId) : undefined
  ...
  if (variant === 'mobile') {
    return (
      <BottomSheet open onClose={close} title={state.mode === 'edit' ? t('form.editTitle') : t('form.addTitle')}>
        {form}
      </BottomSheet>
    )
  }
