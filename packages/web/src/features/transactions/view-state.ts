...
export function parseTransactionsViewState(search: Record<string, unknown>): TransactionsViewState {
  ...
  const month = typeof search.month === 'string' && MONTH_PATTERN.test(search.month)
    ? search.month
    : defaults.month
  const query = typeof search.query === 'string' ? search.query : defaults.query
  const type = typeof search.type === 'string' && TYPE_VALUES.includes(search.type as TransactionFilterType)
    ? (search.type as TransactionFilterType)
    : defaults.type
  const categoryId = typeof search.categoryId === 'string' ? search.categoryId : defaults.categoryId
...
export function buildTransactionsSearch(state: TransactionsViewState): Record<string, string | undefined> {
  const defaults = defaultTransactionsViewState()
  return {
