...
vi.mock('@/features/categories/queries', () => ({
  ...
  useCategoryLookup: () => (id: string | null | undefined) => categories.find((category) => category.id === id),
}))

vi.mock('@/features/accounts/queries', () => ({
  useAccounts: () => ({ data: accounts, isPending: false }),
  useAccountLookup: () => (id: string | null | undefined) => accounts.find((account) => account.id === id),
}))

vi.mock('@/features/categories/components/CategoryFilterSelect', () => ({
  CategoryFilterSelect: ({ ariaLabel }: { ariaLabel: string }) => <div aria-label={ariaLabel}>category-filter</div>,
}))
