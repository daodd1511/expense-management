...
function dueBadge(sub: Subscription) {
  ...
  if (days < 0)
    return { label: `${Math.abs(days)}d overdue`, cls: "bg-expense-muted text-expense" };
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-0.5">
        <div className="flex items-center gap-2">
          <span
            className={cn(
              ...
              !sub.active && "line-through text-muted-foreground",
            )}
          >
            {sub.name}
          </span>
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          {cat && (
            <span className="flex items-center gap-1">
              <span
                ...
                style={{
                  backgroundColor: `color-mix(in oklab, ${colorVar(cat.color)} 20%, transparent)`,
                }}
              >
                <CategoryIcon
                  name={cat.icon}
                  className="size-2.5"
                  style={{ color: colorVar(cat.color) }}
                />
              </span>
              {cat.name}
            </span>
          ...
          <span>
            Ngày {sub.dayOfMonth}
            {sub.cadence === "yearly" ? ` tháng ${sub.monthOfYear}` : ""}
          </span>
          <span>
            {t("sub.nextDue")}: {formatShortDate(sub.nextDueDate)}
          </span>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Drawer open={drawerOpen} onClose={close}>
        <SubscriptionForm initial={editing} onSubmit={handleSubmit} onCancel={close} />
      </Drawer>
      <ConfirmDialog
