...
describe('query invalidation helpers', () => {
  it('invalidates transaction dependents together', async () => {
    ...
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['transactions', 'user-1'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['accounts', 'user-1'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['reports', 'user-1'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['analytics', 'balance-trend', 'user-1'] })
  })
  ...
  it('invalidates category dependents together', async () => {
    ...
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['categories', 'user-1'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['transactions', 'user-1'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['subscriptions', 'user-1'] })
