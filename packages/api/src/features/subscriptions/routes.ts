...
import { logSubscriptionBodySchema, subscriptionCreateSchema, subscriptionPatchSchema } from './schema'
...
subscriptionsRouter.post(
  ...
  async (c) => {
    const data = await controller.logSubscription(c.get('userId'), c.req.param('id'), c.req.valid('json').today)
    return c.json({ data })
  },
)
subscriptionsRouter.patch(
  ...
  async (c) => {
    const data = await controller.updateSubscription(c.get('userId'), c.req.param('id'), c.req.valid('json'))
    return c.json({ data })
  },
)
