...
export async function getDashboardSummary(c: Context<AuthEnv>, query: DashboardSummaryQuery) {
  return c.json(await service.getDashboardSummary(c.get("userId"), query.today));
}

export async function getNetWorthTrend(c: Context<AuthEnv>) {
  return c.json(await service.getNetWorthTrend(c.get("userId"), c.req.query("referenceMonth")));
}
