...
import { transactionBulkDeleteSchema, transactionCreateSchema, transactionPatchSchema } from './schema'
...
transactionsRouter.patch(
  ...
  async (c) => {
    const data = await controller.updateTransaction(c.get('userId'), c.req.param('id'), c.req.valid('json'))
    return c.json({ data })
  },
)
