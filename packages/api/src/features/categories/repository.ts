...
export async function countChildren(categoryId: string) {
  const supabase = getSupabase()
  const result = await supabase.from('categories').select('id', { count: 'exact', head: true }).eq('parent_id', categoryId)
...
export async function clearTransactionsCategory(userId: string, categoryId: string) {
  const supabase = getSupabase()
  const result = await supabase.from('transactions').update({ category_id: null }).eq('category_id', categoryId).eq('owner_id', userId)
...
export async function clearSubscriptionsCategory(userId: string, categoryId: string) {
  const supabase = getSupabase()
  const result = await supabase.from('subscriptions').update({ category_id: null }).eq('category_id', categoryId).eq('owner_id', userId)
...
export async function deleteBudget(userId: string, categoryId: string) {
  const supabase = getSupabase()
  const result = await supabase.from('budgets').delete().eq('category_id', categoryId).eq('owner_id', userId)
